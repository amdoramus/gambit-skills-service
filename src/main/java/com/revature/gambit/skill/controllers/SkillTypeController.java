...
public class SkillTypeController {
     ...
     * @param skillType Incoming data fields will be mapped into this SkillType object.
	 ...
	 * @return Iterable object containing all the skill types retrieved along with HTTP status code 200 (OK)
	 */
	 ...
	 * @return Skill type along with HTTP status code 200 (OK) if found, HTTP
	...
	/**
	 * Handles incoming PUT request that will update an existing skill type with a new one.
	 * @param skillType Existing skill type will be updated with this one.
	 * @param name Name of the skill type to update.
	 * @return HTTP status code 202 (ACCEPTED) if success, HTTP
     * status code 404 (NOT FOUND) otherwise.
	 */
   ...
   /**
    * Handles incoming DELETE request that will remove (soft delete) a skill type from the system.
